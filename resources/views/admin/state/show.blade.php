...
@section('content')
    <div class="container">
        <div class="row">
            ...
            {!! Button::primary($iconEdit)->asLinkTo(route('admin.state.edit', ['state' => $state->id])) !!}
            @php $iconDestroy = Icon::create('remove'); @endphp
            {!! Button::danger($iconDestroy)
                ->asLinkTo(route('admin.state.destroy', ['state' => $state->id]))
                ->addAttributes(['onclick' => "event.preventDefault();document.getElementById(\"form-delete\").submit();"])
            !!}
            @php $formDelete = FormBuilder::plain([
                    'id' => 'form-delete',
                    'route' => ['admin.state.destroy','state' => $state->id],
                    'method' => 'DELETE',
                    'style' => 'display:none'
                ]) @endphp
            {!! form($formDelete) !!}
